...
#define REWARD_CUT_BLOCK 8000250
...
uint256 hashGenesisBlock("0x00000e0fceabd6fb2e406d00a1218d1e4305e0d120ce8202a1e84dd384f8924a");

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Vectrum: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Vectrum Signed Message:\n";
...
{
    // Vectrum: IsDust() detection disabled, allows any valid dust to be relayed.
...
// Launch specs
static const int64 nTargetTimespan = 24 * 60 * 60; // Vectrum: each block
static const int64 nTargetSpacing = 2 * 60; // Vectrum: 120 seconds
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Vectrum: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, Vectrum - DarkGravity, written by Evan Duffield - evan@Vectrum.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Vectrum: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Vectrum: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x000003a985a462230e8d581dca0cbf6977ab6ce570d84e7c921d9b10f4e76efb");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
      ...
      //vMerkleTree: 3f66108684400d84e2ffb528360d534fa19ecaf47c7c4c2ca1d1352ec4a351ad
        ...
        // Genesis block
        const char* pszTimestamp = "United Arab Emirates Financial Authorities Host Crypto Asset and Fintech Forum";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1552902715;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2061696;

        if (fTestNet)
        {
            block.nTime    = 1552902821;
            block.nNonce   = 62468;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xce9b325407d8c80a2668abe0ee79c8bc23e9f7959f1b2b47bec5a02d98293895"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xa9, 0xd5, 0xc6, 0xf5 }; 
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("VectrumMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("VectrumMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static VectrumMiner(CWallet *pwallet)
{
    printf("VectrumMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Vectrum-miner");
    ...
    try { while (true) {
        ...
        printf("Running VectrumMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("VectrumMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&VectrumMiner, pwallet));
}
