    ...
    model(0)
{
    ...
    ui->treeWidget->setColumnWidth(COLUMN_LAVCT, 170);
...
{
    if (ui->radioTreeMode->isChecked() && contextMenuItem->text(COLUMN_LAVCT).length() == 0 && contextMenuItem->parent())
        GUIUtil::setClipboard(contextMenuItem->parent()->text(COLUMN_LAVCT));
    else
        GUIUtil::setClipboard(contextMenuItem->text(COLUMN_LAVCT));
}
...
void CoinControlDialog::updateView()
{
    ...
    BOOST_FOREACH(PAIRTYPE(QString, vector<COutput>) coins, mapCoins)
    {
        ...
        if (treeMode)
        {
            ...
            // label
            itemWalletAddress->setText(COLUMN_LAVCT, sWalletLabel);
        ...
        BOOST_FOREACH(const COutput& out, coins.second)
        {
            ...
            if (!(sAddress == sWalletAddress)) // change
            {
                ...
                itemOutput->setToolTip(COLUMN_LAVCT, tr("change from %1 (%2)").arg(sWalletLabel).arg(sWalletAddress));
                itemOutput->setText(COLUMN_LAVCT, tr("(change)"));
            }
            else if (!treeMode)
            {
                QString sLabel = "";
                ...
                if (sLabel.length() == 0)
                    sLabel = tr("(no label)");
                itemOutput->setText(COLUMN_LAVCT, sLabel); 
            }
